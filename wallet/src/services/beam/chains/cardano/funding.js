 ...
 */
export async function getPureAdaBalance(address) {
  const utxos = await fetchUtxos(address);
  return utxos
    .filter(u => !u.assets || u.assets.length === 0)
 ...
 */
export async function checkCardanoBeamReadiness(address, minFunding = MIN_FUNDING_LOVELACE) {
  const utxos = await fetchUtxos(address);
...
export async function consolidateAdaUtxos({
  address, seedPhrase, addressIndex = 0, excludeUtxoIds = [], onStatus,
}) {
  ...
  await submitCardanoTx(fixedTx.to_bytes());
