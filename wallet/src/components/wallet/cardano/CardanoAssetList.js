...
export default function CardanoAssetList({ assets, onBeamBack, onRedeem }) {
  if (!assets.length) return null;

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {assets.map(asset => (
        <CardanoAssetCard key={asset.unit} asset={asset} onBeamBack={onBeamBack} onRedeem={onRedeem} />
      ))}
    </div>
  );
}
